...
font-family: 'Inter', sans-serif; 
    ...
    @supports (font-variation-settings: normal) {
        .tools { font-family: 'Inter var', sans-serif; }
      }
`;
...
const Designer = ({ location }) => {
    ...
    const { data, error, loading } = useQuery(ORIGIN_BY_ID, { variables: { host: docId } });

    const [debouncedText] = useDebounce(textbox, 100)


    const initConnection = () => {
        const childMessenger = RxPostmessenger.connect(iframe.current.contentWindow, data.origin.protocol + "://" + data.origin.origin);
            ...
            {!loading && (
                <React.Fragment>
                    ...
                    <Content style={{ position: "relative" }}>
                        <Editor>
                            <div style={{ padding: 8 }}>
                                <Iframe ref={iframe} onLoad={initConnection} src={data.origin.protocol + "://" + data.origin.origin} />
                            </div>
